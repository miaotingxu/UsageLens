...
- 右上角提供紧凑的“界面样式”下拉菜单，可在精密仪表、双层玻璃、横向时间轴、极简终端四套 UI 间切换；四套 UI 共用同一份额度、Token、倒计时、刷新和折叠逻辑。
- 所选样式会保存在当前 Windows 用户的 `%LOCALAPPDATA%\CodexQuotaFloat\appearance.json`，仅包含样式名称，不含账号、额度或会话数据。
...
- 每项额度以细进度条或仪表圆弧显示剩余比例，填充长度/弧度对应剩余百分比并沿用四色规则；无数据时只显示中性轨道。
...
额度和 Token 用量在同一张 300×205 像素卡片内上下展示，无需切换页面：上半区保留 5 小时与 7 天额度、重置倒计时和进度条；下半区显示当日、近 7 天、近 30 天的总 Token。悬停 Token 数字可查看输入与输出明细，输入统计包含 cached input。数值统一使用中文数量级，1 万以上显示“万”，达到 1 亿显示“亿”。价格按每次请求实际使用的模型估算：GPT-5.6 Sol 输入 $4、输出 $20；Terra 输入 $2、输出 $12；Luna 输入 $0.20、输出 $1.20；GPT-6 Astra 输入 $8、输出 $40；codex-auto-review 与 Terra 同价（均为每100万 Token）。给定价表未覆盖的模型不计入价格，界面价格后的 `*` 表示存在未计价 Token；仅供参考，不代表实际账单。
...
- 将鼠标移到保留的小横条会展开窗口。
- 在窗口上右键可手动刷新或退出。
- 左键点击右上角当前样式标签可打开 124 像素宽的样式菜单；选择后立即切换并记住偏好。菜单展开期间不会自动折叠窗口。
- 再次启动程序不会创建第二个实例。
